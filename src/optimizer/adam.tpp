...
template <Numeric T>
Adam<T>::Adam(T learning_rate, T beta1, T beta2, T epsilon)
    : Optimizer<T>(learning_rate), beta1(beta1), beta2(beta2), epsilon(epsilon), t(0) {
    weight_m = nullptr;
    weight_v = nullptr;
    bias_m = nullptr;
    bias_v = nullptr;
...
template <Numeric T>
void Adam<T>::step(Tensor<T>& weights, const Tensor<T>& grad_weights, Tensor<T>& bias, const Tensor<T>& grad_bias) {
    t++;
